...
% 【窗口1】 模型动画
figure('Name', '1. Model Animation', 'Color','w','Position',[50 100 600 500]);
...
title('3D 形变仿真 (实时)');
xlim([-0.3 0.3]); ylim([-0.6 0.1]); zlim([-0.3 0.3]); 
...
for frame = 1:total_frames
    ...
    v_rad = -sum(unit_vec .* v_obj_gpu, 2);           
...
% 4. 【新增】相位谱 (FFT of Phase)
L = length(detrended_phase);
Y = fft(detrended_phase);
P2 = abs(Y/L);
P1 = P2(1:floor(L/2)+1);
P1(2:end-1) = 2*P1(2:end-1);
f_axis_spec = fps * (0:(floor(L/2)))/L;
...
% ================= 【窗口2】 RTI 能量图 =================
figure('Name','2. RTI Map','Color','w', 'Position', [700 650 600 400]);
mesh(t_axis, dist_axis, range_abs); 
view(0, 90); % 俯视视角
...
% ================= 【窗口3】 呼吸相位 (时域) =================
figure('Name','3. Phase (Time Domain)','Color','w', 'Position', [700 100 600 250]);
plot(t_axis, detrended_phase, 'LineWidth', 1.5, 'Color', [0 0.447 0.741]);
grid on;
xlabel('时间 (s)'); ylabel('相位 (rad)');
title(['提取的呼吸相位波形 (距离 Bin: ', num2str(global_max_idx), ')']);

% ================= 【窗口4】 相位谱 (频域) =================
figure('Name','4. Phase Spectrum (Freq Domain)','Color','w', 'Position', [700 400 600 250]);
plot(f_axis_spec, P1, 'LineWidth', 1.5, 'Color', [0.635 0.078 0.184]);
grid on;
xlim([0 5]); % 限制显示范围只看低频呼吸部分 (0-5Hz)
