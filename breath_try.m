...
Mix = zeros(N_rx,Nsample, Nchirp, Nframe);  % 复数基带信号（最终从GPU取回）
...
%     pause(1/fps);
    ...
    % ---------------------- 雷达几何与反射计算（GPU） ----------------------
    for i_rx=1:N_rx
        p_radar_gpu(2) = p_radar(2) + (i_rx-1)*dx;
    ...
    %     mix_si(:,frame) = s_i_gpu;%测试是否为多点干扰用
    %     row_idx = find(any(A ~= 0, 2));
...
%     mix_frame_gpu = sum(mix_point_gpu(330:335,:), 1);
...
%% ---------------------- 第一个chirp的距离FFT ----------------------
Mix_gpu = gpuArray(Mix(1,:,1,:));  % 第个chirp
% % --- 加窗（Hamming窗） ---
...
%{//ai生成的另一种解缠绕//可忽略
% 1. 提取复数信号 (Range FFT - 保持不变)
Mix_gpu = gpuArray(Mix(1,:,1,:));
sig_fft1_gpu = fft(Mix_gpu, [], 1);
sig_fft = gather(squeeze(sig_fft1_gpu)); 

MaxIndex2 = 6; % 选择目标距离单元
S_k = sig_fft(MaxIndex2, :); % S_k 是 N_frame x 1 的复数序列

% ---------------------- 替代原有循环 ----------------------

% 2. 提取原始相位 (Phase Extraction)
phi_measured = angle(S_k); % 提取原始的、卷绕的相位序列

% 3. 标准的相位解卷绕 (Standard Phase Unwrapping)
phi_unwrapped = unwrap(phi_measured); 

% 4. 消除初始相位和整体漂移（可选，用于聚焦周期运动）
% 通常减去平均值或应用高通滤波
phi_dc_removed = phi_unwrapped - mean(phi_unwrapped);

% 5. 进行频率分析
figure;
subplot(121)
plot((0:length(phi_unwrapped)-1)*(total_seconds/Nframe), phi_unwrapped,'LineWidth',2)
xlabel("时间 (s)");
ylabel("相位 (rads)");
title("标准解卷绕相位序列");

subplot(122)
% 对解卷绕后的相位进行FFT，提取呼吸/心跳频率
L = length(phi_unwrapped);
Y = abs(fftshift(fft(phi_unwrapped))) / L;
f = (-L/2:L/2-1) * fps/L;

plot(f, Y / max(Y),'LineWidth',2)
axis([0 10 0 1.1]); % 呼吸频率通常低于1 Hz，心跳频率1-2 Hz
xlabel("频率 (Hz)");
ylabel("归一化幅度");
title("解卷绕相位序列频率图");
