...
tau_h    = T_h / 3;
sigma_h  = T_h / 5;
d_h      = 0.05/10;       % 最大形变位移量 [m]
direction = [-0.5, -1.0, 0.0];

% 胸腔影响范围
x_min = -0.18/10; x_max = 0.18/10;
y_min = -0.20/10; y_max = 0;
z_min = -0.2/10; z_max = 0.2/10;
...
Tchirp = 5 * 2 * maxR / c;  % chirp持续时间 [s]
idle_time = 2.5e-6;    % idle时间 [s]
...
xlim(ax, vc(1) + [-0.008 0.008]);
ylim(ax, vc(2) + [-0.008 0.008]);
zlim(ax, vc(3) + [-0.008 0.008]);
% xlim(ax, vc(1) + [-2 2]);
% ylim(ax, vc(2) + [-2 2]);
% zlim(ax, vc(3) + [-2 2]);
drawnow;
