% put_vmd_clean.m  —— put_vmd.m 的【无噪声版】
% =========================================================================
% 自包含脚本 = wanzheng.m 的数字孪生 FMCW 回波仿真 + main_test.m 的 VMD 解耦出图
% ★ 与 put_vmd.m 功能完全一致，唯一区别：add_noise=false（不加复高斯白噪声），
%   图片输出目录岔开到 数字孪生素材_开题报告/图_无噪声/（避免覆盖有噪版图）。
%
...
%
% 输出：PNG（300 dpi）存到  数字孪生素材_开题报告/图_无噪声/（无噪声版；与 put_vmd.m 唯一区别）
%   组合图：fig6 全IMF时域、fig7 全IMF频谱、fig8 呼吸/心搏 2x2
%   fig9 谱峰对照（总体系一归一化，保留心搏/呼吸实际幅度比）
%   独立图：vmd_imf1..7_时域 / _频谱（每模态时域、频谱各一张）
%                vmd_呼吸_时域 / vmd_呼吸_频谱 / vmd_心跳_时域 / vmd_心跳_频谱
%                fig10 雷达距离-时间图（RX1 第一chirp）
...
outdir   = fullfile(thisdir,'数字孪生素材_开题报告','图_无噪声');
...
% ---- 接收机噪声：本文件 = put_vmd.m 的【无噪声版】，唯一区别是下面 add_noise=false。
% 不加复高斯白噪声，图片输出目录岔开到 图_无噪声/。若想临时对比，把 add_noise 改回 true、
% 并改 SNR_dB 即可（届时建议把 outdir 改回 图/）。
add_noise = false;
SNR_dB    = 50;                      % 信噪比 [dB]（仅 add_noise=true 时按 σ_n=RMS/10^(SNR/20) 用到）
rng_seed  = 0;
noise_tag = '无噪声';
if add_noise, noise_tag = ['加噪 SNR=',num2str(SNR_dB),' dB']; end
...
% 即喂给 VMD 的原始相位增量信号（本无噪声版；有噪版见 put_vmd.m），展示分离前整体面貌
...
title(['雷达距离-时间图（RX1 第一chirp，',noise_tag,'）']);
