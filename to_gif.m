% combined_chest_heart_sim_quiver_display.m
% 将胸腔 (breath) 与心脏 (heartbeat) 两个模型合并在一份脚本中
% 用法向量箭头替代 patch 显示（按用户要求替换显示，不改其他功能）
...
x_min_chest = x_min_chest*daxiao_chest;
x_max_chest=x_max_chest*daxiao_chest;
y_min_chest=y_min_chest*daxiao_chest;
y_max_chest=y_max_chest*daxiao_chest;
z_min_chest=z_min_chest*daxiao_chest;
z_max_chest=z_max_chest*daxiao_chest;
...
d_h_heart     = 0.003*daxiao_chest;     % 最大形变位移量 [m]
...
%% ==================== 绘图初始化（使用 quiver3 代替 patch） ====================
% 控制显示稀疏度与箭头长度（你可以调整）
N_display = 1; % 每隔 N_display 个顶点显示一个法向量（可调）
normal_length_scale = 0.01 * daxiao; % 箭头长度缩放（可调）

% 采样顶点索引（胸腔 / 心脏）
sample_verts_idx_chest = 1:N_display:numVerts_chest;
sample_verts_idx_heart = 1:N_display:numVerts_heart;

% 初始顶点位置（用于显示）
verts_chest_show = Vertices_chest(sample_verts_idx_chest, :);
normals_chest_show = vertex_normals_chest(sample_verts_idx_chest, :);

verts_heart_show = Vertices_heart(sample_verts_idx_heart, :);
normals_heart_show = vertex_normals_heart(sample_verts_idx_heart, :);

% 创建图形与 quiver 对象（胸腔半透明背景去掉，改为 quiver）
fig = figure('Name','Chest + Heart + Normals Display','Color',[1 1 1]);
ax = axes('Parent', fig); hold(ax, 'on');

% chest 法向量（蓝色）
p_chest = quiver3(...
    verts_chest_show(:,1), verts_chest_show(:,2), verts_chest_show(:,3), ...
    normals_chest_show(:,1) * normal_length_scale, ...
    normals_chest_show(:,2) * normal_length_scale, ...
    normals_chest_show(:,3) * normal_length_scale, ...
    'b', 'LineWidth', 1.2, 'MaxHeadSize', 0.5, 'AutoScale', 'off', 'Parent', ax);

% heart 法向量（红色）
p_heart = quiver3(...
    verts_heart_show(:,1), verts_heart_show(:,2), verts_heart_show(:,3), ...
    normals_heart_show(:,1) * normal_length_scale, ...
    normals_heart_show(:,2) * normal_length_scale, ...
    normals_heart_show(:,3) * normal_length_scale, ...
    'r', 'LineWidth', 1.2, 'MaxHeadSize', 0.5, 'AutoScale', 'off', 'Parent', ax);

% 统一视图设置（保留你原来的视角 /范围）
axis equal; grid on;
xlabel('X'); ylabel('Y'); zlabel('Z');
view(3);
title('Chest + Heart Normals (quiver3 display)');
set(ax, 'Color', [1 1 1]);
...
for frame = 1:total_frames
    ...
    % 取回 CPU 用于绘图（原脚本两次 gather 我保留一次）
    V_frame = gather(V_frame_gpu);

    % 分割胸腔和心脏顶点（CPU）
    V_frame_chest = V_frame(1:numVerts_chest, :);
    V_frame_heart = V_frame(numVerts_chest+1:end, :);

    % 更新 quiver 的位置与向量（胸腔）
    verts_chest_show = V_frame_chest(sample_verts_idx_chest, :);
    % 注意：法向量我们使用预计算的静态 vertex_normals_chest（若你想在变形后重新计算法向量，可后续改进）
    normals_chest_show = vertex_normals_chest(sample_verts_idx_chest, :) * normal_length_scale;

    set(p_chest, ...
        'XData', verts_chest_show(:,1), 'YData', verts_chest_show(:,2), 'ZData', verts_chest_show(:,3), ...
        'UData', normals_chest_show(:,1), 'VData', normals_chest_show(:,2), 'WData', normals_chest_show(:,3) );

    % 更新 quiver 的位置与向量（心脏）
    verts_heart_show = V_frame_heart(sample_verts_idx_heart, :);
    normals_heart_show = vertex_normals_heart(sample_verts_idx_heart, :) * normal_length_scale;

    set(p_heart, ...
        'XData', verts_heart_show(:,1), 'YData', verts_heart_show(:,2), 'ZData', verts_heart_show(:,3), ...
        'UData', normals_heart_show(:,1), 'VData', normals_heart_show(:,2), 'WData', normals_heart_show(:,3) );

    title(ax, sprintf('Frame %d / %d (M_chest=%.3f, M_heart=%.3f)', ...
        frame, total_frames, M_t_chest, M_t_heart));
    drawnow;
end

%% ==================== 原始 read_stl_generic 函数（保留原样） ====================
