...
for i=1:numVerts_chest
    ...
    A_chest(i) = d_h_chest * sqrt_fz * 1 * log_term*sqrt_fy;
end
...
%% ==================== 绘图初始化 ====================
%% ==================== 绘图初始化 ====================
%% ==================== 绘图初始化（使用顶点法向量 quiver3 显示） ====================
fig = figure('Name','Chest + Heart Animation with Radar Echo','Color',[1 1 1]);
ax = axes('Parent', fig);
hold(ax, 'on');

% ==================== 显示胸腔法向量 ====================
N_display_chest = 1;   % 每隔 N 个顶点显示一个法向量
sample_verts_idx_c = 1:N_display_chest:numVerts_chest;

verts_to_show_c = Vertices_chest(sample_verts_idx_c, :);  % 顶点
normals_to_show_c = vertex_normals_chest(sample_verts_idx_c, :);  % 法向量

normal_length_scale_chest = 0.01;
normal_length_scale_heart = 0.001;

q_chest = quiver3(verts_to_show_c(:,1), verts_to_show_c(:,2), verts_to_show_c(:,3), ...
                  normals_to_show_c(:,1)*normal_length_scale_chest, ...
                  normals_to_show_c(:,2)*normal_length_scale_chest, ...
                  normals_to_show_c(:,3)*normal_length_scale_chest, ...
                  'Color', [0.6, 0.8, 1.0],'LineWidth',1.2,'MaxHeadSize',0.5,'AutoScale','off');
N_display_heart = 1;   % 每隔 N 个顶点显示一个法向量
% ==================== 显示心脏法向量 ====================
sample_verts_idx_h = 1:N_display_heart:numVerts_heart;

verts_to_show_h = Vertices_heart(sample_verts_idx_h, :);
normals_to_show_h = vertex_normals_heart(sample_verts_idx_h, :);

q_heart = quiver3(verts_to_show_h(:,1), verts_to_show_h(:,2), verts_to_show_h(:,3), ...
                  normals_to_show_h(:,1)*normal_length_scale_heart, ...
                  normals_to_show_h(:,2)*normal_length_scale_heart, ...
                  normals_to_show_h(:,3)*normal_length_scale_heart, ...
                  'r','LineWidth',1.2,'MaxHeadSize',0.5,'AutoScale','off');

axis equal;
axis vis3d;
hold(ax, 'off');
axis(ax, 'off');          % 隐藏坐标轴、刻度、边框
...
% 
% 
% % ---------------- 固定相机：计算模型中心并显式设置 camera ----------------
% % 以 Vertices_all 为依据（你的脚本已有该变量）
% xmin = min(Vertices_all(:,1)); xmax = max(Vertices_all(:,1));
% ymin = min(Vertices_all(:,2)); ymax = max(Vertices_all(:,2));
% zmin = min(Vertices_all(:,3)); zmax = max(Vertices_all(:,3));
% 
% % 模型中心
% cx = 0.5*(xmin + xmax);
% cy = 0.5*(ymin + ymax);
% cz = 0.5*(zmin + zmax);
% 
% % 推荐的相机距离（相对于模型最大维度放大系数 ）
% model_size = max([xmax-xmin, ymax-ymin, zmax-zmin]);
% cam_distance = 5 * model_size;   % 可调：增大值会把相机拉远，看到更多内容
% 
% % 把相机放在 +X 方向（你要从 +X 看向 -X），相机位置设置为 (cx + cam_distance, cy, cz)
% campos(ax, [cx + cam_distance, cy, cz]);
% 
% % 相机目标指向模型中心，保证中心固定
...
% camup(ax, [0 0 1]);
% 
% % 锁定相机视角和位置（防止 drawnow/自动缩放改变）
% set(ax, 'CameraViewAngleMode', 'manual');
% set(ax, 'CameraPositionMode', 'manual');
% set(ax, 'CameraTargetMode', 'manual');
% 
% % 可选：控制视野角（数值越小越“放大”）
% camva(ax, 12);  % 你可以改为 8 / 12 等看效果
% 
% % 最后确保 axis manual（锁定 axis limits，避免动画期间自动变）
% axis(ax, 'manual');
...
for frame = 1:total_frames
    ...
    % --- 更新胸腔 quiver ---
verts_to_show_c = V_chest(sample_verts_idx_c, :);
set(q_chest, 'XData', verts_to_show_c(:,1), ...
             'YData', verts_to_show_c(:,2), ...
             'ZData', verts_to_show_c(:,3), ...
             'UData', normals_to_show_c(:,1)*normal_length_scale_chest, ...
             'VData', normals_to_show_c(:,2)*normal_length_scale_chest, ...
             'WData', normals_to_show_c(:,3)*normal_length_scale_chest);

% --- 更新心脏 quiver ---
verts_to_show_h = V_heart(sample_verts_idx_h, :);
set(q_heart, 'XData', verts_to_show_h(:,1), ...
             'YData', verts_to_show_h(:,2), ...
             'ZData', verts_to_show_h(:,3), ...
             'UData', normals_to_show_h(:,1)*normal_length_scale_heart, ...
             'VData', normals_to_show_h(:,2)*normal_length_scale_heart, ...
             'WData', normals_to_show_h(:,3)*normal_length_scale_heart);
