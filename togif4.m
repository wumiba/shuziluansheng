...
%% ==================== 绘图初始化（使用 patch 面片渲染显示） ====================
fig = figure('Name','Chest + Heart Animation with Radar Echo','Color',[1 1 1]);
ax = axes('Parent', fig);
hold(ax, 'on');

% ----- 胸腔 patch -----
p_chest = patch('Faces', Faces_chest, ...
                'Vertices', Vertices_chest, ...
                'FaceVertexCData', Vertices_chest(:,3), ... % 用 Z 显示渐变
                'FaceColor', 'interp', ...
                'EdgeColor', 'k', ...
                'FaceAlpha', 0.35, ...   % 透明度可调
                'LineWidth', 0.2);

% ----- 心脏 patch -----
p_heart = patch('Faces', Faces_heart, ...
                'Vertices', Vertices_heart, ...
                'FaceVertexCData', Vertices_heart(:,3), ...
                'FaceColor', 'interp', ...
                'EdgeColor', 'k', ...
                'FaceAlpha', 0.5, ...
                'LineWidth', 0.25);
...
set(p_chest, 'Vertices', V_chest, ...
             'FaceVertexCData', V_chest(:,3));

% 更新心脏 patch 顶点
set(p_heart, 'Vertices', V_heart, ...
             'FaceVertexCData', V_heart(:,3));
