...
tau_h    = T_h / 3;
sigma_h  = T_h / 5;
d_h      = 0.05/1;       % 最大形变位移量 [m]
direction = [-0.5, -1.0, 0.0];


% 心脏影响范围
x_min = -0.13/1; x_max = 0.13/1;
y_min = -0.2/1; y_max = 0;
z_min = -0.2/1; z_max = 0.2/1;
...
% ---------- 新增开关与参数（放在参数区） ----------
use_ESC = true;        % true: 使用等效散射中心 (ESC) 替代逐顶点相加； false: 使用原始每点模型
ESC_count = 5;         % ESC 个数（常用 3~10）
use_weighting = false; % true: 在 s_i 上施加胸腔/心脏权重（若合并场景时很有用）
w_chest = 1.0;         % 胸腔权重（当 use_weighting==true）
w_heart = 0.3;         % 心脏权重（当 use_weighting==true）
% ----------------------------------------------------
...
%**********************************************
%% -------------------- 输入 --------------------
% Vertices       原始顶点坐标矩阵 (N x 3)
% Faces          原始三角面片矩阵 (M x 3)
% vertex_normals 原始顶点法向量矩阵 (N x 3)
...
%% -------------------- Step 1: 体素哈希 --------------------
% 每个点所在体素的索引 (整数坐标)
voxel_idx = floor(Vertices ./ v);

% key: 'ix_iy_iz'
keys = strcat( ...
    num2str(voxel_idx(:,1)), '_', ...
    num2str(voxel_idx(:,2)), '_', ...
    num2str(voxel_idx(:,3)) );

% 用 Map 存储 {点集合, 法向量集合, 旧索引}
voxel_map = containers.Map();

for i = 1:size(Vertices,1)
    key = keys(i,:);
    if isKey(voxel_map,key)
        data = voxel_map(key);
        data{1} = [data{1}; Vertices(i,:)];       % 点
        data{2} = [data{2}; vertex_normals(i,:)]; % 法向量
        data{3} = [data{3}; i];                   % 原始索引
        voxel_map(key) = data;
    else
        voxel_map(key) = {Vertices(i,:), vertex_normals(i,:), i};
    end
end
...
Vertices_new = [];
vertex_normals_new = [];
old2new = zeros(size(Vertices,1),1);

all_keys = voxel_map.keys;
for k = 1:length(all_keys)
    ...
    % 平均法向量 (归一化)
    ni_bar = mean(nms,1);
    ni_bar = ni_bar ./ norm(ni_bar);

    % 存入新矩阵
    Vertices_new(end+1,:) = pi_bar;
    vertex_normals_new(end+1,:) = ni_bar;

    % 映射旧点到新点索引
    old2new(old_ids) = size(Vertices_new,1);
end

%% -------------------- Step 3: Faces_new --------------------
Faces_new = old2new(Faces);

% 删除退化面（三个点有重复）
mask = (Faces_new(:,1) ~= Faces_new(:,2)) & ...
       (Faces_new(:,1) ~= Faces_new(:,3)) & ...
       (Faces_new(:,2) ~= Faces_new(:,3));
Faces_new = Faces_new(mask,:);


Vertices=Vertices_new;
vertex_normals = vertex_normals_new;
Faces = Faces_new;
numFaces = size(Faces,1);
numVerts = size(Vertices,1);
n_sum = zeros(numVerts,3);
...
if use_ESC
    % 选择 ESC_count 个散射中心的简单策略：
    % 方案：PCA 投影 + 分段采样 + 法向量能量加权（简洁鲁棒）
    V_all = Vertices; % Nx3
    Nverts_local = size(V_all,1);

    % PCA 主方向
    [coeff, score, ~] = pca(V_all);

    % 在第一主成分方向上均匀选择 ESC_count 个位置：
    proj = V_all * coeff(:,1); % 投影坐标
    p_min = min(proj); p_max = max(proj);
    % 等分位置
    tvec = linspace(p_min, p_max, ESC_count);

    ESC = zeros(ESC_count,3);
    ESC_w = zeros(ESC_count,1);
    for k = 1:ESC_count
        % 找到最接近该投影位置的原顶点索引
        [~, idx] = min(abs(proj - tvec(k)));
        ESC(k,:) = V_all(idx,:);
        % 初始权重可基于该点法向量与雷达方向的 Lambert 投影平均
        nvec = vertex_normals(idx,:);
        rdr_dir = (p_radar - ESC(k,:));
        rdr_dir = rdr_dir / (norm(rdr_dir) + 1e-12);
        lam = max(0, -dot(rdr_dir, nvec)); % 类Lambert系数（标量）
        ESC_w(k) = max(1e-3, lam); % 防止为0
    end

    % 归一化权重到均值1（便于调参）
    ESC_w = ESC_w / mean(ESC_w);

    % GPU 备份
    ESC_gpu = gpuArray(ESC);
    ESC_w_gpu = gpuArray(ESC_w);
end
...
plot((-length(aaa)/2:length(aaa)/2-1)*fps/length(aaa),abs(fftshift(fft(aaa)))/max(abs(fftshift(fft(aaa)))),'LineWidth',2)
axis([0 10 0 1.1]);
xlabel("频率 (Hz)");
ylabel("幅度");
title("差分相位序列频率图");
...
% 使用 ESC 替代顶点集合
    % ESC_gpu: [M x 3], ESC_w_gpu: [M x 1]
    % 计算 r0, v, s_i 对 ESC（GPU 上）
    % 注意：保持时间轴 t_frame_fmcw_gpu 一致
    d_i_ESC_gpu = ESC_gpu - p_radar_gpu;                 % M x 3 (静态位置)
    r0_ESC_gpu  = sqrt(sum((d_i_ESC_gpu).^2, 2));        % M x 1

    % ------- 应用运动位移 (近似，全局 Z 位移) -------
    if exist('delta_max_all_gpu','var')
        try
            % 计算当前帧的位移（coeffs_gpu * delta_max_all_gpu）
            delta_tmp_gpu = coeffs_gpu .* delta_max_all_gpu; % numVerts x 3
            mean_dz_gpu   = mean(delta_tmp_gpu(:,3));        % 全局平均 Z 位移
        catch
            mean_dz_gpu   = gpuArray(0);
        end
    else
        mean_dz_gpu = gpuArray(0);
    end
    ...
    % ESC 位置随帧位移
    ESC_pos_frame_gpu = ESC_gpu;
    if abs(gather(mean_dz_gpu)) > 1e-12
        ESC_pos_frame_gpu = ESC_pos_frame_gpu + ...
            [zeros(size(ESC_gpu,1),2,'gpuArray'), ...
             mean_dz_gpu*ones(size(ESC_gpu,1),1,'gpuArray')];
    end

    % ------- 重新计算 r0 (frame) -------
    d_i_ESC_frame_gpu = ESC_pos_frame_gpu - p_radar_gpu; % M x 3
    r0_point_gpu      = sqrt(sum(d_i_ESC_frame_gpu.^2,2)); % M x 1

    % 近似径向速度 v_i = 0 （ESC 代表簇中心）
    v_point_gpu = gpuArray(zeros(size(r0_point_gpu)));

    % Lambertian + 权重
    rdr_dir_ESC_gpu = d_i_ESC_frame_gpu ./ (r0_point_gpu + epsilon);
    lam_ESC_gpu     = max(0, -sum(rdr_dir_ESC_gpu .* ...
                           gpuArray(vertex_normals(1:size(ESC_gpu,1),:)), 2));
    s_i_ESC_gpu     = ESC_w_gpu .* lam_ESC_gpu;

    % ------- 信号计算 -------
    r_t_gpu    = r0_point_gpu + v_point_gpu .* t_frame_fmcw_gpu;  
    td_gpu     = 2 * r_t_gpu / c;
    phi_tx_gpu = 2*pi*(fc*t_frame_fmcw_gpu + 0.5*slope*t_frame_fmcw_gpu.^2);
    phi_rx_gpu = 2*pi*(fc*(t_frame_fmcw_gpu - td_gpu) + ...
                       0.5*slope*(t_frame_fmcw_gpu - td_gpu).^2);

    Tx_gpu        = exp(1j * phi_tx_gpu);         % 1 x numSamples
    Rx_gpu        = exp(1j * phi_rx_gpu) .* s_i_ESC_gpu;  % M x numSamples
    mix_point_gpu = Tx_gpu .* conj(Rx_gpu);       % M x numSamples
    mix_frame_gpu = sum(mix_point_gpu, 1);        % 1 x numSamples
