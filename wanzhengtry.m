...
% 额外：对每个顶点引入面积权重（面面积/3 的累加），用于面积归一化反射强度
% 注意：其余参数与原脚本完全保持不变
...
heart_stl = 'b.stl';   % 心脏 STL（若你原来用 b.stl 可改回）
...
use_vertex_normals_heart = false; % 心脏原脚本 false (沿固定方向扩张)
...
%% ==================== 体素 / 平滑 等（如需请在此插入） ====================
% 我没有自动执行体素重采样；如果你之前运行过体素步骤，请在这里插入。
% 为保持与原始代码一致，本脚本不执行体素重采样.
...
%% ==================== —— 新增：按面面积分配到顶点，用作面积归一化权重 —— %%
% 计算胸腔各面面积并累加到对应顶点（面积/3）
vertex_area_chest = zeros(numVerts_chest,1);
for f = 1:size(Faces_chest,1)
    ids = Faces_chest(f,:);
    v1 = Vertices_chest(ids(1),:);
    v2 = Vertices_chest(ids(2),:);
    v3 = Vertices_chest(ids(3),:);
    face_area = 0.5 * norm(cross(v2 - v1, v3 - v1));
    vertex_area_chest(ids) = vertex_area_chest(ids) + face_area/3;
end

% 计算心脏各面面积并累加到对应顶点（面积/3）
vertex_area_heart = zeros(numVerts_heart,1);
for f = 1:size(Faces_heart,1)
    ids = Faces_heart(f,:);
    v1 = Vertices_heart(ids(1),:);
    v2 = Vertices_heart(ids(2),:);
    v3 = Vertices_heart(ids(3),:);
    face_area = 0.5 * norm(cross(v2 - v1, v3 - v1));
    vertex_area_heart(ids) = vertex_area_heart(ids) + face_area/3;
end

% 合并为整体顶点面积权重向量（与 Vertices_all 顺序一致）
vertex_area_all = [vertex_area_chest; vertex_area_heart*10000];

% 将面积权重移到 GPU，用于在 s_i_gpu 上加权（面积归一化）
% 注意：不对面积做额外缩放/归一（保持物理比例）
vertex_area_all_gpu = gpuArray(vertex_area_all);
...
for frame = 1:total_frames
    ...
    % ========= 在 s_i_gpu 上乘以面积权重（面积归一化） =========
    % 这一步保留物理比例，不改变你的参数或主流程。面积权重已移到 GPU。
    s_i_gpu = s_i_gpu .* vertex_area_all_gpu;  % per-vertex area scales the reflection
